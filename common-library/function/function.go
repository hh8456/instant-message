...
var (
	closeLoggerChan chan bool
)

func init() {
	closeLoggerChan = make(chan bool)
}
...
func backtrace(message interface{}) {
	...
	log.Fatalf("Traceback[%s] (most recent call last):\n", time.Now())
	for i := 0; ; i++ {
...
LOOP:
	...
	for len(buff[index:]) >= int(packet.PacketMaxSize_16K) {
		select {
...
func ReadPacket2(conn net.Conn, reader *bufio.Reader) ([]byte, error) {
	...
	if dataBodyLen <= uint32(1024*1024-packet.PacketHeaderSize) {
		conn.SetReadDeadline(time.Now().Add(time.Minute * 2))
		buf := make([]byte, packet.PacketHeaderSize+int(dataBodyLen))
		copy(buf, headBuf)
		_, err := io.ReadFull(reader, buf[packet.PacketHeaderSize:packet.PacketHeaderSize+int(dataBodyLen)])
		if err != nil {
			return nil, err
		}
		return buf, nil
...
func ReadPacket_(reader *bufio.Reader, buf []byte) (int, error) {
	// 先读包头
	...
	if dataBodyLen <= uint32(1024*1024-packet.PacketHeaderSize) {
		...
		if err != nil {
			...
			return packet.PacketHeaderSize + int(dataBodyLen), nil
		}
	} else {
